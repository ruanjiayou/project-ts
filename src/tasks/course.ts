...
const task = new Task('1 0 0 * * *', createCourse, null, true, 'Asia/Chongqing');
createCourse();
async function createCourse() {
  try {
    ...
    while (today.toDate().getTime() < lastDay.toDate().getTime()) {
      let day = today.day();
      const schedule = await models.Schedule.findOne({ where: { startTime: { lte: today.startOf('day').toDate() }, endTime: { gte: today.endOf('day').toDate() } } });
      let bSpecialMode = schedule ? true : false;
      if (day === 0 || day === 6 || bSpecialMode) {
